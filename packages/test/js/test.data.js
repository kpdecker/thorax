$(function(){
  ...
  var loadStart = 'load:start',
      ...
      Application = new Thorax.Application(),
      exports = Application;
  ...
  test('views should see load start from model', function() {
    var spy = this.spy(),
        model = new Application.Model({url: 'foo'}),
        view = new Application.View({name: 'food', render: function(){}, model: model});
  ...
  test('views should see load end from model', function() {
    var model = new Application.Model({url: 'foo'}),
        view = new Application.View({name: 'food', render: function(){}, model: model}),
  ...
  QUnit.module('thorax.loading - root load events', {
    setup: function() {
      ...
      exports.bind('load:start', Thorax.loadHandler(this.startSpy, this.endSpy));
  
      this.model = new Application.Model({url: 'foo'});
      this.view = new Application.View({name: 'food', model: this.model, render: function(){}});
    },
    teardown: function() {
      exports._loadStart = undefined;
      exports.unbind('load:start');
      exports.unbind('load:end');
    }
  });
  ...
  QUnit.module('thorax.loading - event throttle', {
    setup: function() {
      ...
      var object = this.object = _.extend({}, Backbone.Events, Thorax.LoadableMixin);
  ...
  QUnit.module('thorax.loading - forwardLoadEvents', {
    setup: function() {
      ...
      this.dest.bind('load:start', this.startSpy);
  
      this.src = _.extend({}, Backbone.Events, Thorax.LoadableMixin);
    }
  });
  ...
  test('unbind clears bind', function() {
  ...
  QUnit.module('thorax.loading - data.load', {
    setup: function() {
      ...
      this.model = new (Application.Model.extend({url: 'foo'}))();
      this.model.bind(loadStart, this.startSpy);
      this.model.bind(loadEnd, this.endSpy);
    }
  });
  ...
  test('data load sends events to root', function() {
    ...
    exports.bind('load:start', rootStart);
    this.model.load(success, failback);
    this.requests[0].respond(200, {}, '{}');
    ...
    exports.unbind('load:start', rootStart);
  });
  test('data load sends events to root - concurrent background', function() {
    ...
    this.model.bind('load:start', Thorax.loadHandler(rootStart, rootEnd));
    this.model.load(success, failback, {background: true});
    this.clock.tick(1000);
  ...
  test("bindToRoute", function() {
    var callback,
        ...
        _Router = Application.Router.extend({}),
        router = new _Router();
  ...
  test("loading helper and loading collection options", function() {
    var loadingView = new Application.View({
    ...
    loadingView = new Application.View({
      template: '{{#loading}}<p>loading</p>{{else}}<p>not loading</p>{{/loading}}'
    });
    loadingView.loadStart();
    this.clock.tick(loadStartTimeout);
    ...
    var loadingViewWithModel = new Application.View({
      template: '{{#loading}}<p>loading</p>{{else}}<p>not loading</p>{{/loading}}',
    ...
    var loadingViewWithModelExplicitlyCalled = new Application.View({
